...
def back(img_no):
    ...
    label = Label(image=List_images[img_no - 1])
    label.grid(row=1, column=0, columnspan=3)
    button_forward = Button(root, text="submit",
                            command=lambda: forward(img_no + 1))
    button_back = Button(root, text="Back",
                         command=lambda: back(img_no - 1))
    print(img_no)
...
image_no_1 = ImageTk.PhotoImage(Image.open("captcha.png"))
image_no_2 = ImageTk.PhotoImage(Image.open(image_path))
image_no_3 = ImageTk.PhotoImage(Image.open('test.png'))
image_no_4 = ImageTk.PhotoImage(Image.open(image_path))
